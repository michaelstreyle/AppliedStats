...
First, we read in the data and do some cleaning.
...
#plot with modeled and age and survival
newdat <- data.frame(ag=seq(min(agegroup), max(agegroup),len=1313))
newdat$sur = predict(ship.logit, newdata=newdat, type="response")
plot(newdat$sur~newdat$ag, data=ship, col="red4", main='Ship.logit Predictions by Age Group')
lines(lowess(newdat$sur ~ newdat$ag))



#plot of pclass vs survival with ship.logit model
newdat1 <- data.frame(pc=seq(min(pclass), max(pclass),len=1313))
newdat1$sur = predict(ship.logit, newdata=newdat1, type="response")
plot(newdat1$sur~newdat1$pc, data=ship, col="red4", main='Ship.logit Predictions by Pclass')
lines(lowess(newdat1$sur ~ newdat1$pc, newdat1),col="green4")

#Code from Dr. Phil
pi1 = glm (survived ~ agegroup + sex + pclass, family=binomial)
summary(pi1)
pi.ageseq = seq (min(agegroup), max(agegroup), len=1313)
pi1.pred = predict (pi1, data.frame (ag=pi.ageseq), type='response')
plot (pi1.pred~pi.ageseq, col=sex)
lines (lowess (pi1.pred~pi.ageseq))
plot (pi1.pred, col=sex)  # plots predictions vs row number
#end of Dr. Phil Code

agegpseq = seq(1,9, len=30)
pi.pred2 = predict(pi1, data.frame(agegroup=agegpseq, sex='male', pclass=1), type='response')
plot(agegroup, jitter(survived, 0.2), col=sex)
lines(agegpseq, pi.pred2)

pi.pred3 = predict(pi1, data.frame(agegroup=agegpseq, sex='male', pclass=3), type='response')
lines(agegpseq, pi.pred3, col='red')

pi.pred4 = predict(pi1, data.frame(agegroup=agegpseq, sex='male', pclass=2), type='response')
lines(agegpseq, pi.pred4, col='blue')
...
#plot with modeled and age and survival
newdat3 <- data.frame(ag=seq(min(agegroup), max(agegroup),len=1313))
newdat3$sur = predict(ship.logit.step, newdata=newdat3, type="response")
plot(newdat3$sur~newdat3$ag, data=ship, col="red4", main='Ship.logit.step Predictions by Age Group')
lines(lowess(newdat3$sur ~ newdat3$ag, newdat3),col="green4")
...
exp(fm$coefficients)
#plot with modeled and age and survival
newdat4 <- data.frame(ag=seq(min(agegroup), max(agegroup),len=1313))
newdat4$sur = predict(fm, newdata=newdat4, type="response")
plot(newdat4$sur~newdat4$ag, data=ship, col="red4", main='Final Model Predictions by Age Group')
lines(lowess(newdat4$sur ~ newdat4$ag, newdat4, lwd=2),col="green4")
```

Next, we plot some of the fitted probabilities for each pclass against age and sex. 
These plots are useful for seeing the differences between men and womens survival amongst the passenger classes. If you want to survive, it is best to be a middle aged woman in the first passenger class. 
...
The following is the function form Dr. Phil to check our ROC related calculations
...
roc.table = roc.logistic (ship.logit)
summary(ship.logit)
