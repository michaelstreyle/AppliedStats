...
ds <- read.table(file.choose("CH01PR19.txt"),header=F,col.names=c("GPA","ACT"))
...
t.test(fit, "sig.level" = 0.01)
cat("\nHYPOTHESIS TESTS FOR INTERCEPT AND SLOPE:\n\n")
summary(fit)$coefficients

cat("\nCONFIDENCE INTERVAL FOR A MEAN RESPONSE:\n\n")
predict(fit, data.frame(ACT=28), interval="confidence", se.fit=TRUE, level=0.9)$fit
